...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (typeof window === "undefined") return; // فقط سمت کلاینت اجرا بشه
  ...
  const handleUsernameLogin = async () => {
    try {
        ...
        },
        {
          headers: {
            ...
            RequestVerificationToken:
              "CfDJ8HUnEMJS3ptJkVwmIv5DWiu4R-o9uKPacx35zAeaMgR1OJQXMBW_MA4SFpQvYucqrogV_06nXmVaVUWf4b53BypzwJq-8tRQivPGESt861mpquRYeyFIN6Cyi76uJiZt9-3rdgqtJBqqhMW7aPdX8u",
      ...
      if (response.data?.result === 1) {
        console.log("Username login successful, fetching token...");
        ...
        setToast({ message: "نام کاربری یا رمز عبور اشتباه است ❌", type: "error" });
      }
    } catch (error) {
        ...
        <div className="w-full max-w-lg bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl p-8 mt-8 transition-all duration-200 hover:scale-[1.02] hover:shadow-3xl">
          ...
          {tab === "username" ? (
            <>
              ...
              />
              <button
                className="w-full py-3 rounded-xl bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500 text-white font-extrabold text-sm shadow-md transition-all duration-200 hover:scale-[1.05] hover:opacity-90"
